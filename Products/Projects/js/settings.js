...
ASC.Projects.SettingsManager = (function () {
    ...
    function showEntityMenu(selectedActionCombobox) {
        ...
        var menuItems = [
            new ActionMenuItem("status_edit", commonResource.Edit, saEditHandler.bind(null, status))
        ];

        if (status.id > 0) {
            menuItems.push(new ActionMenuItem("status_delete", commonResource.Delete, saDeleteHandler.bind(null, status)));
        }
    ...
    function saEditHandler(status) {
        ...
        StudioBlockUIManager.blockUI($commonPopupContainer, 400, 550, 0, "absolute");
